...
class StesiTable {
    ...
    return data;
	       ...
	       processing: true,
			...
			fixedHeader:true,
			serverSide : true,';
		if(!empty($this->displayLength)){
			$table.="displayLength:".$this->displayLength.",";
		}
		if(!empty($this->displayStart)){
			$table.="displayStart:".$this->displayStart.",";
		}
...
\"<'row'<'col-sm-8'li><'col-sm-4'p>>";
    ...
    data: function ( d ) {
		 ...
		 */
		foreach ( $tableColums as $column ) {
			// Se � nascosta, la colonna non deve essere visualizzata
			if (! $column->isHidden ()) {
